...
const MypageBookmark = () => {
    ...
    const [isSelected, setIsSelected] = useState<{ id: string; title: string; price: string; imageUrl: string; }[]>([]);
    const [isFavorite, setIsFavorite] = useState<{ id: string; title: string; price: string; imageUrl: string; }[]>([]);
    ...
    const handleDeleteItem = (productId: string) => {
        const handleRealDelete = () => {
            setIsFavorite((prev) => {
                const updatedFavorites = prev.filter((product) => product.id !== productId) //삭제하려는 아이템의 아이디를 지움
                if (userId) {
                    localStorage.setItem(`favorite_${userId}`, JSON.stringify(updatedFavorites));
                }
            ...
            {isFavorite.length !== 0 ? (
                <div>
                        ...
                        <ProductTableMenu >
                            {isFavorite.map((product) => (
                                <div key={product.id} className={tableStyles.content}>
                                    ...
                                    <ProductTableMenu.Detail onClick={() => navigate('/add-review')} productName={product.title} />
                    ...
                    <div className={cartStyles.button_box}>
                        <Button className={cartStyles.bookmark_button} label='장바구니에 담기' onClick={handleAddCart} />
                    </div>
                </div>) : (
                <div>
                    <MypageHeader />
